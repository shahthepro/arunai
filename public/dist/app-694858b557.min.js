...
!function(app){"use strict";app.registerModule("assesments"),app.registerModule("assesments.admin.routes")}(ApplicationConfiguration);
...
!function(app){"use strict";app.registerModule("core",["ui.toggle","angular-loading-bar"]),app.registerModule("core.routes",["ui.router"]),app.registerModule("core.admin",["core","ui.toggle"]),app.registerModule("core.admin.routes",["ui.router"])}(ApplicationConfiguration);
...
!function(app){"use strict";app.registerModule("sidebars",["ui.tinymce","ui.sortable"])}(ApplicationConfiguration);
...
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("adminbar",{title:"Assesments",state:"admin.assesments",type:"dropdown",roles:["professor","hod","student"],position:2}),menuService.addSubMenuItem("adminbar","admin.assesments",{title:"Internal Assesments",state:"admin.assesments.list",roles:["professor","hod"]}),menuService.addSubMenuItem("adminbar","admin.assesments",{title:"Internal Assesments",state:"admin.assesments.view",roles:["student"],position:2})}angular.module("assesments").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("admin.assesments",{abstract:!0,url:"/assesments",template:"<ui-view/>"}).state("admin.assesments.list",{url:"",templateUrl:"/modules/assesments/client/views/list-assesments.client.view.html",controller:"AssesmentsListController",controllerAs:"vm",data:{roles:["professor","admin"],pageTitle:"Internal Assesments"}}).state("admin.assesments.mark",{url:"/:assignmentId/mark",templateUrl:"/modules/assesments/client/views/mark-assesment.client.view.html",controller:"MarkAssesmentController",controllerAs:"vm",resolve:{assignmentResolve:getAssignment},data:{roles:["professor","admin"],pageTitle:"Update Internal Marks"}}).state("admin.assesments.report",{url:"/:assignmentId/report",templateUrl:"/modules/assesments/client/views/report-assesment.client.view.html",controller:"ReportAssesmentController",controllerAs:"vm",resolve:{assignmentResolve:getAssignment},data:{roles:["professor","admin"],pageTitle:"Assesment Report"}}).state("admin.assesments.view",{url:"/view",templateUrl:"/modules/assesments/client/views/view-assesment.client.view.html",controller:"ViewAssesmentController",controllerAs:"vm",data:{roles:["student"],pageTitle:"Assesment Report"}})}function getAssignment($stateParams,AssignmentsService){return AssignmentsService.get({assignmentId:$stateParams.assignmentId}).$promise}function getCourse($stateParams,CoursesService){return CoursesService.get({courseId:$stateParams.courseId}).$promise}angular.module("assesments.admin.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getAssignment.$inject=["$stateParams","AssignmentsService"],getCourse.$inject=["$stateParams","CoursesService"]}();
!function(){"use strict";function AssesmentsListController(AssesmentsService,Authentication,ProfessorsService){var vm=this;vm.authentication=Authentication,vm.assignments=ProfessorsService.assignedCourses({userId:vm.authentication.user._id}),vm.assesments=AssesmentsService.query()}angular.module("assesments").controller("AssesmentsListController",AssesmentsListController),AssesmentsListController.$inject=["AssesmentsService","Authentication","ProfessorsService"]}();
!function(){"use strict";function MarkAssesmentController(AssesmentsService,ProfessorsService,StudentsService,Authentication,$http,$state,assignment,Notification){var vm=this;vm.authentication=Authentication,vm.professor=vm.authentication.user,vm.assignment=assignment,vm.course=vm.assignment.course,vm.assesmentTest="cia1",vm.assesments=[],vm.isLoading=!1,vm.fetchAssesments=function(){vm.assesments=[],vm.isLoading=!0;var assesmentPromise=AssesmentsService.getAssesments({batch:vm.assignment.batch,courseId:vm.course._id,test:vm.assesmentTest}).$promise,studentsPromise=StudentsService.filterStudents({departmentId:vm.course.department,batch:vm.assignment.batch,semester:vm.course.semester}).$promise;Promise.all([assesmentPromise,studentsPromise]).then(function(resps){var ind,assesments=resps[0],students=resps[1];for(vm.assesments=assesments,ind=0;ind<assesments.length;ind++)AssesmentsService.get({assesmentId:assesments[ind]._id});if(vm.assesments.length<=0){for(ind=0;ind<students.length;ind++){var newAssesment=new AssesmentsService;newAssesment.student=students[ind],newAssesment.course=vm.course,newAssesment.test=vm.assesmentTest,newAssesment.score=0,vm.assesments.push(newAssesment)}Notification.info({message:"Assesment not yet marked for the test",delay:3e3})}vm.isLoading=!1}).catch(function(reason){vm.isLoading=!1,Notification.error({title:"Something went wrong :(",message:reason,delay:3e3})})},vm.fetchAssesments(),vm.saveAssesment=function(){function successCallback(res){}function errorCallback(res){}vm.isLoading=!0;var assesmentPromises=[];vm.assesments.forEach(function(assesment){assesment._id?assesmentPromises.push(assesment.$update(successCallback,errorCallback).$promise):assesmentPromises.push(assesment.$save(successCallback,errorCallback).$promise)}),Promise.all(assesmentPromises).then(function(resps){Notification.success({message:"Internal Assesment records were saved!",delay:3e3}),vm.isLoading=!1}).catch(function(reason){Notification.error({message:"Something went wrong :(",delay:3e3}),vm.isLoading=!1})}}angular.module("assesments").controller("MarkAssesmentController",MarkAssesmentController),MarkAssesmentController.$inject=["AssesmentsService","ProfessorsService","StudentsService","Authentication","$http","$state","assignmentResolve","Notification"]}();
!function(){"use strict";function ReportAssesmentController(AssesmentsService,ProfessorsService,StudentsService,Authentication,$http,$state,assignment,Notification){var vm=this;vm.authentication=Authentication,vm.professor=vm.authentication.user,vm.assignment=assignment,vm.course=vm.assignment.course,vm.assesmentTest="cia1",vm.assesments={},vm.isLoading=!1,vm.fetchAssesments=function(){vm.assesments={},vm.isLoading=!0;var assesmentPromise=AssesmentsService.getAssesments({batch:vm.assignment.batch,courseId:vm.course._id,test:vm.assesmentTest}).$promise;Promise.all([assesmentPromise]).then(function(resps){var assesments=resps[0];assesments.forEach(function(item){AssesmentsService.get({assesmentId:item._id})}),vm.assesments=assesments,vm.isLoading=!1}).catch(function(reason){Notification.error({title:"Something went wrong :(",message:reason,delay:3e3}),vm.isLoading=!1})}}angular.module("assesments").controller("ReportAssesmentController",ReportAssesmentController),ReportAssesmentController.$inject=["AssesmentsService","ProfessorsService","StudentsService","Authentication","$http","$state","assignmentResolve","Notification"]}();
!function(){"use strict";function ViewAssesmentController(AssesmentsService,Authentication){function groupByTest(array,f){var groups=[];return array.forEach(function(o){var group=f(o);groups[group]=groups[group]||[],groups[group].name=group,groups[group][o.test]=o.score}),Object.keys(groups).map(function(group){return groups[group]})}var vm=this;vm.authentication=Authentication,vm.isLoading=!0,vm.assesments=[],AssesmentsService.getByStudent({studentId:vm.authentication.user._id}).$promise.then(function(assesments){vm.assesments=groupByTest(assesments,function(item){return item.course.code}),vm.isLoading=!1}).catch(function(reason){Notification.error({message:"Something went wrong :(",delay:3e3})})}angular.module("assesments").controller("ViewAssesmentController",ViewAssesmentController),ViewAssesmentController.$inject=["AssesmentsService","Authentication"]}();
!function(){"use strict";function AssesmentsService($resource){return $resource("/api/assesments/:assesmentId",{assesmentId:"@_id"},{update:{method:"PUT"},getAssesments:{method:"GET",url:"/api/assesments/:courseId/:batch/:test",isArray:!0},getByStudent:{method:"GET",url:"/api/assesments/bystudent/:studentId",isArray:!0}})}angular.module("assesments").factory("AssesmentsService",AssesmentsService),AssesmentsService.$inject=["$resource"]}();
...
!function(){"use strict";function menuConfig(menuService){menuService.addSubMenuItem("adminbar","admin.assesments",{title:"Daily Attendance",state:"admin.attendances.list",roles:["professor","hod"]}),menuService.addSubMenuItem("adminbar","admin.assesments",{title:"Attendance",state:"admin.attendances.view",roles:["student"],position:2})}angular.module("attendances").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("admin.attendances",{abstract:!0,url:"/attendances",template:"<ui-view/>"}).state("admin.attendances.list",{url:"",templateUrl:"/modules/attendances/client/views/list-attendances.client.view.html",controller:"AttendancesListController",controllerAs:"vm",data:{roles:["professor","admin"],pageTitle:"Daily Attendance"}}).state("admin.attendances.mark",{url:"/:assignmentId/mark",templateUrl:"/modules/attendances/client/views/mark-attendance.client.view.html",controller:"MarkAttendanceController",controllerAs:"vm",resolve:{assignmentResolve:getAssignment},data:{roles:["professor","admin"],pageTitle:"Mark Attendance"}}).state("admin.attendances.report",{url:"/:assignmentId/report",templateUrl:"/modules/attendances/client/views/report-attendance.client.view.html",controller:"ReportAttendanceController",controllerAs:"vm",resolve:{assignmentResolve:getAssignment},data:{roles:["professor","admin"],pageTitle:"Attendance Report"}}).state("admin.attendances.view",{url:"/view",templateUrl:"/modules/attendances/client/views/view-attendance.client.view.html",controller:"ViewAttendanceController",controllerAs:"vm",data:{roles:["student"],pageTitle:"Attendance Report"}})}function getAssignment($stateParams,AssignmentsService){return AssignmentsService.get({assignmentId:$stateParams.assignmentId}).$promise}function getCourse($stateParams,CoursesService){return CoursesService.get({courseId:$stateParams.courseId}).$promise}angular.module("attendances.admin.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getAssignment.$inject=["$stateParams","AssignmentsService"],getCourse.$inject=["$stateParams","CoursesService"]}();
...
!function(){"use strict";function menuConfig(menuService){}angular.module("core.admin").run(menuConfig),menuConfig.$inject=["menuService"]}();
...
!function(){"use strict";function menuConfig(menuService,PagesService){menuService.addMenu("account",{roles:["user"]}),menuService.addMenuItem("account",{title:"Account",state:"settings",type:"dropdown",roles:["user","student","professor","admin","hod"]}),menuService.addSubMenuItem("account","settings",{title:"My Dashboard",state:"admin.dashboard"}),PagesService.getForMenu().$promise.then(function(pages){pages.forEach(function(page){menuService.addMenuItem("topbar",{title:page.title,state:'pages.view({ slug: "'+page.slug+'" })',position:page.menuOrder})})})}angular.module("core").run(menuConfig),menuConfig.$inject=["menuService","PagesService"]}();
...
!function(){"use strict";function FooterController(){var vm=this;vm.year=(new Date).getFullYear()}angular.module("core").controller("FooterController",FooterController)}();
...
!function(){"use strict";function HomeController(PagesService){var vm=this;vm.events=PagesService.getEvents()}angular.module("core").controller("HomeController",HomeController),HomeController.$inject=["PagesService"]}();
...
!function(){"use strict";function pageTitle($rootScope,$interpolate,$state){function link(scope,element){function listener(event,toState){var applicationCoreTitle="Arunai Engineering College",separeteBy=" - ";if(toState.data&&toState.data.pageTitle){var stateTitle=$interpolate(toState.data.pageTitle)($state.$current.locals.globals);element.html(applicationCoreTitle+separeteBy+stateTitle)}else element.html(applicationCoreTitle)}$rootScope.$on("$stateChangeSuccess",listener)}var directive={restrict:"A",link:link};return directive}function pageHeader($rootScope,$interpolate,$state){function link(scope,element){function listener(event,toState){var applicationCoreTitle="Arunai Engineering College";if(toState.data&&toState.data.pageTitle){var stateTitle=$interpolate(toState.data.pageTitle)($state.$current.locals.globals);element.html(stateTitle)}else element.html(applicationCoreTitle)}$rootScope.$on("$stateChangeSuccess",listener)}var directive={restrict:"A",link:link};return directive}angular.module("core").directive("pageTitle",pageTitle).directive("pageHeader",pageHeader),pageTitle.$inject=["$rootScope","$interpolate","$state"],pageHeader.$inject=["$rootScope","$interpolate","$state"]}();
...
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("admin.assignments",{abstract:!0,url:"/assignments",template:"<ui-view/>"}).state("admin.assignments.list",{url:"",templateUrl:"/modules/courses/client/views/list-assignments.client.view.html",controller:"AssignmentsListController",controllerAs:"vm",data:{roles:["admin","hod"],pageTitle:"Course Assignments"}}).state("admin.assignments.create",{url:"/create",templateUrl:"/modules/courses/client/views/form-assignment.client.view.html",controller:"AssignmentsController",controllerAs:"vm",resolve:{assignmentResolve:newAssignment},data:{roles:["admin","hod"],pageTitle:"Assign Courses"}}).state("admin.assignments.edit",{url:"/:assignmentId/edit",templateUrl:"/modules/courses/client/views/form-assignment.client.view.html",controller:"AssignmentsController",controllerAs:"vm",resolve:{assignmentResolve:getAssignment},data:{roles:["admin","hod"],pageTitle:"Update Course Assignment"}})}function getAssignment($stateParams,AssignmentsService){return AssignmentsService.get({assignmentId:$stateParams.assignmentId}).$promise}function newAssignment(AssignmentsService){return new AssignmentsService}angular.module("courses.admin.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getAssignment.$inject=["$stateParams","AssignmentsService"],newAssignment.$inject=["AssignmentsService"]}();
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("adminbar",{title:"Courses",state:"admin.courses",type:"dropdown",roles:["professor","hod","admin"],position:4}),menuService.addSubMenuItem("adminbar","admin.courses",{title:"Add Courses",state:"admin.courses.create",roles:["professor","hod","admin"]}),menuService.addSubMenuItem("adminbar","admin.courses",{title:"List Courses",state:"admin.courses.list",roles:["professor","hod","admin"]})}angular.module("courses").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("admin.courses",{abstract:!0,url:"/courses",template:"<ui-view/>"}).state("admin.courses.list",{url:"",templateUrl:"/modules/courses/client/views/list-courses.client.view.html",controller:"CoursesListController",controllerAs:"vm",data:{roles:["admin","hod"],pageTitle:"Courses"}}).state("admin.courses.create",{url:"/create",templateUrl:"/modules/courses/client/views/form-course.client.view.html",controller:"CoursesController",controllerAs:"vm",resolve:{courseResolve:newCourse},data:{roles:["admin","hod"],pageTitle:"Add Course"}}).state("admin.courses.edit",{url:"/:courseId/edit",templateUrl:"/modules/courses/client/views/form-course.client.view.html",controller:"CoursesController",controllerAs:"vm",resolve:{courseResolve:getCourse},data:{roles:["admin","hod"],pageTitle:"Edit Course {{ courseResolve.name }}"}})}function getCourse($stateParams,CoursesService){return CoursesService.get({courseId:$stateParams.courseId}).$promise}function newCourse(CoursesService){return new CoursesService}angular.module("courses.admin.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getCourse.$inject=["$stateParams","CoursesService"],newCourse.$inject=["CoursesService"]}();
...
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("adminbar",{title:"Departments",state:"admin.departments",type:"dropdown",roles:["admin"],position:3}),menuService.addSubMenuItem("adminbar","admin.departments",{title:"Add Departments",state:"admin.departments.create",roles:["admin"]}),menuService.addSubMenuItem("adminbar","admin.departments",{title:"List Departments",state:"admin.departments.list",roles:["admin"]})}angular.module("departments").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("admin.departments",{abstract:!0,url:"/departments",template:"<ui-view/>"}).state("admin.departments.list",{url:"",templateUrl:"/modules/departments/client/views/list-departments.client.view.html",controller:"DepartmentsListController",controllerAs:"vm",data:{roles:["admin"],pageTitle:"Departments"}}).state("admin.departments.create",{url:"/create",templateUrl:"/modules/departments/client/views/form-department.client.view.html",controller:"DepartmentsController",controllerAs:"vm",resolve:{departmentResolve:newDepartment},data:{roles:["admin"],pageTitle:"Add Department"}}).state("admin.departments.edit",{url:"/:departmentId/edit",templateUrl:"/modules/departments/client/views/form-department.client.view.html",controller:"DepartmentsController",controllerAs:"vm",resolve:{departmentResolve:getDepartment},data:{roles:["admin"],pageTitle:"Edit Department {{ departmentResolve.name }}"}})}function getDepartment($stateParams,DepartmentsService){return DepartmentsService.get({departmentId:$stateParams.departmentId}).$promise}function newDepartment(DepartmentsService){return new DepartmentsService}angular.module("departments.admin.routes").config(routeConfig),routeConfig.$inject=["$stateProvider"],getDepartment.$inject=["$stateParams","DepartmentsService"],newDepartment.$inject=["DepartmentsService"]}();
...
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("adminbar",{title:"Pages",state:"admin.pages",roles:["admin"],type:"dropdown",position:1}),menuService.addSubMenuItem("adminbar","admin.pages",{title:"All Pages",state:"admin.pages.list",roles:["admin"],position:1}),menuService.addSubMenuItem("adminbar","admin.pages",{title:"New Page",state:"admin.pages.create",roles:["admin"],position:2})}angular.module("pages").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("pages",{abstract:!0,url:"/pages",template:"<ui-view/>"}).state("admin.pages",{abstract:!0,url:"/pages",template:"<ui-view/>"}).state("admin.pages.list",{url:"",templateUrl:"/modules/pages/client/views/list-pages.client.view.html",controller:"PagesListController",controllerAs:"vm",data:{pageTitle:"Pages"}}).state("admin.pages.create",{url:"/create",templateUrl:"/modules/pages/client/views/form-page.client.view.html",controller:"PagesController",controllerAs:"vm",resolve:{pageResolve:newPage},data:{roles:["user","admin"],pageTitle:"New Page"}}).state("admin.pages.edit",{url:"/:pageId/edit",templateUrl:"/modules/pages/client/views/form-page.client.view.html",controller:"PagesController",controllerAs:"vm",resolve:{pageResolve:getPage},data:{roles:["user","admin"],pageTitle:"Edit Page {{ pageResolve.title }}"}}).state("pages.view",{url:"/:slug",templateUrl:"/modules/pages/client/views/view-page.client.view.html",controller:"PagesController",controllerAs:"vm",resolve:{pageResolve:getPageBySlug},data:{pageTitle:"{{ pageResolve.title }}"}})}function getPage($stateParams,PagesService){return PagesService.get({pageId:$stateParams.pageId}).$promise}function getPageBySlug($stateParams,PagesService){return PagesService.getBySlug({slug:$stateParams.slug}).$promise}function newPage(PagesService){return new PagesService}getPageBySlug.$inject=["$stateParams","PagesService"],angular.module("pages").config(routeConfig),routeConfig.$inject=["$stateProvider"],getPage.$inject=["$stateParams","PagesService"],newPage.$inject=["PagesService"]}();
...
!function(){"use strict";function PagesController($scope,$state,$window,Authentication,page,SidebarsService){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.page.$remove($state.go("pages.list"))}function save(isValid){function successCallback(res){$state.go("pages.view",{slug:res.slug})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.page._id?vm.page.$update(successCallback,errorCallback):vm.page.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.pageForm"),!1)}var vm=this;vm.authentication=Authentication,vm.page=page,vm.error=null,vm.form={},vm.remove=remove,vm.save=save,vm.sidebars=SidebarsService.query(),vm.tinymceOptions={plugins:"link image code",toolbar:"undo redo | styleselect formatselect fontselect fontsizeselect | bold italic underline strikethrough | alignleft aligncenter alignright alignjustify | bullist numlist | image | outdent indent | code"}}angular.module("pages").controller("PagesController",PagesController),PagesController.$inject=["$scope","$state","$window","Authentication","pageResolve","SidebarsService"]}();
...
!function(){"use strict";function PagesService($resource){return $resource("/api/pages/:pageId",{pageId:"@_id"},{update:{method:"PUT"},query:{method:"GET",isArray:!0},getBySlug:{method:"GET",url:"/api/pages/slugged/:slug"},getForMenu:{method:"GET",url:"/api/pages/onmenu",isArray:!0},getEvents:{method:"GET",url:"/api/pages/events",isArray:!0}})}angular.module("pages").factory("PagesService",PagesService),PagesService.$inject=["$resource"]}();
!function(){"use strict";function menuConfig(menuService){menuService.addSubMenuItem("adminbar","admin.pages",{title:"All Sidebars",state:"admin.sidebars.list",roles:["admin"],position:3}),menuService.addSubMenuItem("adminbar","admin.pages",{title:"New Sidebar",state:"admin.sidebars.create",roles:["admin"],position:4})}angular.module("pages").run(menuConfig),menuConfig.$inject=["menuService"]}();
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("admin.sidebars",{abstract:!0,url:"/sidebars",template:"<ui-view/>"}).state("admin.sidebars.list",{url:"",templateUrl:"/modules/sidebars/client/views/list-sidebars.client.view.html",controller:"SidebarsListController",controllerAs:"vm",data:{roles:["admin"],pageTitle:"Sidebars"}}).state("admin.sidebars.create",{url:"/create",templateUrl:"/modules/sidebars/client/views/form-sidebar.client.view.html",controller:"SidebarsController",controllerAs:"vm",roles:["admin"],resolve:{sidebarResolve:newSidebar},data:{roles:["admin"],pageTitle:"New Sidebar"}}).state("admin.sidebars.edit",{url:"/:sidebarId/edit",templateUrl:"/modules/sidebars/client/views/form-sidebar.client.view.html",controller:"SidebarsController",controllerAs:"vm",resolve:{sidebarResolve:getSidebar},data:{roles:["admin"],pageTitle:"Edit Sidebar {{ sidebarResolve.name }}"}})}function getSidebar($stateParams,SidebarsService){return SidebarsService.get({sidebarId:$stateParams.sidebarId}).$promise}function newSidebar(SidebarsService){return new SidebarsService}angular.module("sidebars").config(routeConfig),routeConfig.$inject=["$stateProvider"],getSidebar.$inject=["$stateParams","SidebarsService"],newSidebar.$inject=["SidebarsService"]}();
...
!function(){"use strict";function SidebarsController($scope,$state,$window,Authentication,sidebar){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.sidebar.$remove($state.go("admin.sidebars.list"))}function save(isValid){function successCallback(res){$state.go("admin.sidebars.list")}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.sidebar._id?vm.sidebar.$update(successCallback,errorCallback):vm.sidebar.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.sidebarForm"),!1)}function addWidget(){vm.sidebar.widgets=vm.sidebar.widgets||[],vm.sidebar.widgets.push({title:"New Sidebar Widget",content:"Widget content goes here..."})}function removeWidget(index){vm.sidebar.widgets.splice(index,1)}var vm=this;vm.authentication=Authentication,vm.sidebar=sidebar,vm.error=null,vm.form={},vm.remove=remove,vm.save=save,vm.addWidget=addWidget,vm.removeWidget=removeWidget,vm.tinymceOptions={plugins:"link image code",toolbar:"undo redo | styleselect formatselect fontselect fontsizeselect | bold italic underline strikethrough | alignleft aligncenter alignright alignjustify | bullist numlist | image | outdent indent | code"}}angular.module("sidebars").controller("SidebarsController",SidebarsController),SidebarsController.$inject=["$scope","$state","$window","Authentication","sidebarResolve"]}();
...
!function(){"use strict";function menuConfig(menuService){menuService.addMenuItem("adminbar",{title:"Settings",state:"settings",type:"dropdown",position:9}),menuService.addSubMenuItem("adminbar","settings",{title:"Edit Profile",state:"settings.profile",position:1}),menuService.addSubMenuItem("adminbar","settings",{title:"Change Password",state:"settings.password",position:2})}angular.module("users").run(menuConfig),menuConfig.$inject=["menuService"]}();
