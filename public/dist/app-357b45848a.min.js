...
!function(){"use strict";function PagesController($scope,$state,$window,Authentication,page){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.page.$remove($state.go("pages.list"))}function save(isValid){function successCallback(res){$state.go("pages.view",{slug:res.slug})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.page._id?vm.page.$update(successCallback,errorCallback):vm.page.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.pageForm"),!1)}var vm=this;vm.authentication=Authentication,vm.page=page,vm.error=null,vm.form={},vm.remove=remove,vm.save=save,vm.tinymceOptions={plugins:"link image code",toolbar:"undo redo | styleselect formatselect fontselect fontsizeselect | bold italic underline strikethrough | alignleft aligncenter alignright alignjustify | bullist numlist | image | outdent indent | code"}}angular.module("pages").controller("PagesController",PagesController),PagesController.$inject=["$scope","$state","$window","Authentication","pageResolve"]}();
