...
!function(app){"use strict";app.registerModule("slides")}(ApplicationConfiguration);
...
!function(){"use strict";function routeConfig($stateProvider){$stateProvider.state("admin.slides",{abstract:!0,url:"/slides",template:"<ui-view/>"}).state("admin.slides.list",{url:"",templateUrl:"modules/slides/client/views/list-slides.client.view.html",controller:"SlidesListController",controllerAs:"vm",data:{pageTitle:"Slides List"}}).state("admin.slides.create",{url:"/create",templateUrl:"modules/slides/client/views/form-slide.client.view.html",controller:"SlidesController",controllerAs:"vm",resolve:{slideResolve:newSlide},data:{roles:["user","admin"],pageTitle:"Slides Create"}}).state("admin.slides.edit",{url:"/:slideId/edit",templateUrl:"modules/slides/client/views/form-slide.client.view.html",controller:"SlidesController",controllerAs:"vm",resolve:{slideResolve:getSlide},data:{roles:["user","admin"],pageTitle:"Edit Slide {{ slideResolve.name }}"}})}function getSlide($stateParams,SlidesService){return SlidesService.get({slideId:$stateParams.slideId}).$promise}function newSlide(SlidesService){return new SlidesService}angular.module("slides").config(routeConfig),routeConfig.$inject=["$stateProvider"],getSlide.$inject=["$stateParams","SlidesService"],newSlide.$inject=["SlidesService"]}();
!function(){"use strict";function SlidesListController(SlidesService){var vm=this;vm.slides=SlidesService.query()}angular.module("slides").controller("SlidesListController",SlidesListController),SlidesListController.$inject=["SlidesService"]}();
!function(){"use strict";function SlidesController($scope,$state,$window,Authentication,slide){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.slide.$remove($state.go("slides.list"))}function save(isValid){function successCallback(res){$state.go("slides.view",{slideId:res._id})}function errorCallback(res){vm.error=res.data.message}return isValid?void(vm.slide._id?vm.slide.$update(successCallback,errorCallback):vm.slide.$save(successCallback,errorCallback)):($scope.$broadcast("show-errors-check-validity","vm.form.slideForm"),!1)}var vm=this;vm.authentication=Authentication,vm.slide=slide,vm.error=null,vm.form={},vm.remove=remove,vm.save=save}angular.module("slides").controller("SlidesController",SlidesController),SlidesController.$inject=["$scope","$state","$window","Authentication","slideResolve"]}();
!function(){"use strict";function SlidesService($resource){return $resource("api/slides/:slideId",{slideId:"@_id"},{update:{method:"PUT"}})}angular.module("slides").factory("SlidesService",SlidesService),SlidesService.$inject=["$resource"]}();
